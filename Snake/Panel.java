...
public class Panel extends JPanel implements ActionListener{
    ...
    final int x[] = new int[GAME_UNITS];        //coordinates
    ...
    public void draw(Graphics g) {
        if(running){
            ...
            g.drawString("Score: " + applesEaten, (SCREEN_WIDTH - metrics.
    ...
    public void checkCollisions() {
        for(int i = bodyParts; 0 < i; i--){           //head collides
            if((x[0] == x[i]) && (y[0] == y[i])){
                running = false;
            }
        }
        if(x[0] < 0){                                            //head touches left border
            running = false;
        }
        if(x[0] > SCREEN_WIDTH){                    //head touches right border
            running = false;
        }
    ...
    public void gameOver(Graphics g) {
        g.setColor(Color.RED);
            ...
            g.drawString("Score: " + applesEaten, (SCREEN_WIDTH - metrics1.
                stringWidth("Score: " + applesEaten))/2, g.getFont().getSize());
